...
- **Player**: MVP é polling do manifest com `ETag` (D6). Fase 2 adiciona MQTT para push de comandos e invalidação; o polling permanece como rede de segurança. Quando este documento diz "evento realtime para o player", leia: *bump de versão do manifest hoje, push MQTT amanhã*.
...
1. Nenhum asset é *tocável* sem: status `ready` **e** metadados mínimos completos **e** pelo menos uma licença válida (verificação delegada ao Licensing).
